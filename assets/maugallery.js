/* gestion du clic au niveau de la barre de nav */
...
itemConcert.addEventListener('click', () =>{
    removeActive()
    ...
    contentConcert.forEach(els =>{
        els.style.display = 'block'
    })
    contentConcert.forEach(els =>{
        els.style.display = 'block';

        for(let i = 0; i < contentConcert.length; i++){
        contentConcert.forEach(els =>{
            let A = els.children
...
itemEntrepriser.addEventListener('click', () =>{
    removeActive()
    ...
    contentEntreprises.forEach(els =>{
        els.style.display = 'block'; 
    })

    for(let i = 0; i < contentEntreprises.length; i++){
        contentEntreprises.forEach(els =>{
            let A = els.children   
                A.src = els.children[i].src
                tabImg.push(A)
...
itemMariage.addEventListener('click', () =>{
    removeActive()
    ...
    tabImg = []
    contentMariages.forEach(els =>{
    removeActive()

        els.style.display = 'block';

        for(let i = 0; i < contentMariages.length; i++){
        contentMariages.forEach(els =>{
            let A = els.children
                A.src = els.childNodes[i].src
                A.alt = els.childNodes[i].alt
...
itemPortrait.addEventListener('click', ()=>{
    removeActive()
    ...
    tabImg = []
    contentProtraits.forEach(els =>{
        els.style.display = 'block';

        for(let i = 0; i < contentProtraits.length; i++){
        contentProtraits.forEach(els =>{
            let A = els.children
                A.src = els.childNodes[i].src
                A.alt = els.childNodes[i].alt
...
/* ici gestion du clic sur la boite modale */
function gestionClicModale(){

  containerSlider.addEventListener('click', ()=>{
    containerSlider.style.display = 'none'
})

boiteSlider.addEventListener('click', (e)=>{
    e.stopPropagation()
})  
}
gestionClicModale()

/* ici ajout de la valeur de l'image à la modale */

galleryItems.forEach((element, id) =>{
    element.addEventListener('click', ()=>{
        /* ici on affciche l'image dans la modale */
...
btnRight.addEventListener('click', ()=>{
    index = (index < tabImg.length - 1) ? index + 1 : 0;
    imgSlider.src = tabImg[index].src;
})

btnLeft.addEventListener('click', ()=>{
    index = (index > 0) ? index - 1 : tabImg.length - 1;
    imgSlider.src = tabImg[index].src;
})
